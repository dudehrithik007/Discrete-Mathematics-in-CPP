...
elements. That's why these 3 subsets are combination of 3 'ELEMENTS' taken 2 at a time. Permutation of bits as each bit pattern (is unique)
in that order selects different different elements as the elements in the set are in order in the array in which they were inputted.) of 
size n (n bits can represent binary numbers from 0 to 2^n - 1), where inside any subset a 0 will indicate that that corresponding element
from the set(whose subsets we are finding here) won't be present in the current subset and a 1 indicates that it'll be present (Here order
of the elements stays same in a set as they're inputted in an array. So, 'corresponding element' makes sense here - only to read 
(corresponding) elements from the set according to the current binary number).

*/

// The number of permutations of n objects taken r at time with repeition = n^r
// 2^3 = 8 permutations of 2 objects taken 3 at a time ofc with repeition.